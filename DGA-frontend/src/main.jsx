// src/main.jsx (Single-File Component: Merged App Logic and Styles)

import React, { useState } from 'react';
import ReactDOM from 'react-dom/client';

// ------------------------------------
// 1. STYLES COMPONENT (รวมจาก index.css)
// ------------------------------------
const AppStyles = () => (
    <style>
        {`
...
// ------------------------------------
// 2. APP COMPONENT (รวมจาก App.jsx)
// ------------------------------------

// ⭐️ Environment Variable Check (Fallback method) ⭐️
// พยายามดึง VITE_BACKEND_URL จาก globalThis (สำหรับ Docker/Vite) หรือใช้ localhost เป็นค่า default
const BACKEND_URL = globalThis.VITE_BACKEND_URL || 'http://localhost:1040'; 
...
function App() {
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            const loginResult = await fetchApi(`${BACKEND_URL}/api/login`, {
                ...
                // Backend Stateless: ส่งข้อมูลผู้ใช้กลับมาโดยตรง
                body: JSON.stringify({ appId: mockAppId, mToken: mToken, token: token }), 
            });

            // ⭐️ บันทึกข้อมูลผู้ใช้ลง State
            setUserData(loginResult.user);
            setStatus('Login และดึงข้อมูลสำเร็จ! บันทึกข้อมูลลง MongoDB แล้ว');
    ...
    const handleLogout = async () => {
        ...
        try {
            // เรียก Stateless Logout (แค่ล้าง Client-side State)
            ...
            {userData && (
                <>
                    ...
                    <div className="section mt-6">
                        <h2 className="text-xl font-semibold mb-3 text-gray-800">2. User Data (Stateless)</h2>
